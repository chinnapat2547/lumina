    ...
    if ($stmtUser = mysqli_prepare($conn, $sqlUser)) {
        ...
        if (mysqli_num_rows($resultUser) === 1) {
            ...
            if (password_verify($password, $rowUser['u_password']) || $password === $rowUser['u_password']) {
                ...
                // 🔴 แก้ไขตรงนี้: อย่าเพิ่ง header() ไปทันที ให้ตั้งค่า alert ก่อน
                $alertType = "success";
                $alertMsg  = "ยินดีต้อนรับ " . $rowUser['u_username'] . " เข้าสู่ระบบเรียบร้อยแล้ว";
                
                // header("Location: ../home.php"); // <-- เอาอันนี้ออก หรือ comment ไว้
                // exit; // <-- เอาออกเพื่อให้โค้ดไหลไปทำงานส่วน HTML/JS ด้านล่าง
