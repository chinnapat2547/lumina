...
<div class="flex min-h-screen w-full">
    ...
    <main class="flex-1 flex flex-col min-w-0">
        ...
        <div class="p-6 lg:p-8 flex flex-col gap-6 max-w-[1600px] mx-auto w-full">
            ...
            <div class="mb-4">
                <h1 class="text-3xl font-extrabold text-gray-800 tracking-tight flex items-center gap-3">ตั้งค่าระบบ (Settings)</h1>
                    ...
                    <div id="tab-store" class="tab-content bg-white rounded-[2rem] shadow-card border border-pink-50 p-8 <?= $activeTab=='store'?'active':'' ?>">
                        ...
                        <form action="" method="POST" class="space-y-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div><label class="block text-sm font-bold text-gray-700 mb-2">ชื่อร้านค้า (Store Name)</label><input type="text" name="store_name" value="Lumina Beauty" class="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 focus:bg-white focus:ring-2 focus:ring-primary/30 outline-none transition-all"></div>
                                <div><label class="block text-sm font-bold text-gray-700 mb-2">เบอร์โทรศัพท์ติดต่อ</label><input type="text" name="store_phone" value="081-234-5678" class="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 focus:bg-white focus:ring-2 focus:ring-primary/30 outline-none transition-all"></div>
                            </div>
                            <div><label class="block text-sm font-bold text-gray-700 mb-2">อีเมลสำหรับติดต่อลูกค้า</label><input type="email" name="store_email" value="support@luminabeauty.com" class="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 focus:bg-white focus:ring-2 focus:ring-primary/30 outline-none transition-all"></div>
                            <div><label class="block text-sm font-bold text-gray-700 mb-2">ที่อยู่ร้านค้า</label><textarea name="store_address" rows="3" class="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 focus:bg-white focus:ring-2 focus:ring-primary/30 outline-none transition-all resize-none">123 หมู่ 4 ถนนความสวย แขวงน่ารัก เขตสดใส กรุงเทพมหานคร 10110</textarea></div>
                        ...
                        <form action="" method="POST" class="space-y-8">
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 bg-gray-50/50 p-5 rounded-2xl border border-gray-100">
                                    <div><label class="block text-xs font-bold text-gray-500 mb-1">ยอดขั้นต่ำสำหรับส่งฟรี</label><div class="relative"><span class="absolute left-4 top-2.5 text-gray-400 font-bold">฿</span><input type="number" value="1000" class="w-full pl-10 pr-4 py-2.5 rounded-xl border border-gray-200 bg-white focus:ring-2 focus:ring-primary/30 outline-none font-bold"></div></div>
                                    <div><label class="block text-xs font-bold text-gray-500 mb-1">ค่าส่งแบบธรรมดา</label><div class="relative"><span class="absolute left-4 top-2.5 text-gray-400 font-bold">฿</span><input type="number" value="50" class="w-full pl-10 pr-4 py-2.5 rounded-xl border border-gray-200 bg-white focus:ring-2 focus:ring-primary/30 outline-none"></div></div>
                                    <div><label class="block text-xs font-bold text-gray-500 mb-1">ค่าส่งด่วน (Express)</label><div class="relative"><span class="absolute left-4 top-2.5 text-gray-400 font-bold">฿</span><input type="number" value="100" class="w-full pl-10 pr-4 py-2.5 rounded-xl border border-gray-200 bg-white focus:ring-2 focus:ring-primary/30 outline-none"></div></div>
                                </div>
                            </div>
                            <div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-50/50 p-5 rounded-2xl border border-gray-100">
                                    <div><label class="block text-xs font-bold text-gray-500 mb-1">เลือกธนาคาร</label><select class="w-full px-4 py-2.5 rounded-xl border border-gray-200 bg-white outline-none"><option value="kbank">กสิกรไทย</option><option value="scb">ไทยพาณิชย์</option></select></div>
                                    <div><label class="block text-xs font-bold text-gray-500 mb-1">เลขบัญชี</label><input type="text" value="123-4-56789-0" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 bg-white font-mono outline-none"></div>
                                </div>
                            </div>
                    ...
                    <div id="tab-admin" class="tab-content bg-white rounded-[2rem] shadow-card border border-pink-50 p-8 <?= $activeTab=='admin'?'active':'' ?>">
                        ...
                        <form action="" method="POST" class="space-y-6">
                            ...
                            <div class="bg-purple-50/30 p-6 rounded-2xl border border-purple-100/50 mb-6 flex items-center gap-4">
                                ...
                                <div><p class="text-xs text-gray-500 mb-1">ผู้ดูแลระบบปัจจุบัน</p><p class="text-lg font-bold text-gray-800"><?= htmlspecialchars($adminName) ?></p></div>
                            </div>
                            <div class="max-w-md space-y-6">
                                <div><label class="block text-sm font-bold text-gray-700 mb-2">ชื่อผู้ดูแลระบบ</label><input type="text" name="admin_username" value="<?= htmlspecialchars($adminName) ?>" required class="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 focus:ring-2 focus:ring-purple-400/30 outline-none"></div>
                                <div class="pt-4 border-t border-gray-100">
                                    <h3 class="text-sm font-bold text-gray-800 mb-4 flex items-center gap-2"><span class="material-icons-round text-orange-500 text-[18px]">lock</span> เปลี่ยนรหัสผ่าน</h3>
                                    <div class="space-y-4">
                                        <div><label class="block text-xs font-bold text-gray-500 mb-1">รหัสผ่านใหม่ (ปล่อยว่างถ้าไม่เปลี่ยน)</label><input type="password" name="new_password" class="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 outline-none"></div>
                                        <div><label class="block text-xs font-bold text-gray-500 mb-1">ยืนยันรหัสผ่านใหม่</label><input type="password" name="confirm_password" class="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-50 outline-none"></div>
                                    </div>
                                </div>
                            </div>
...
<script>
    ...
    <?php if (isset($_SESSION['error_msg'])): ?>
        Swal.fire({ toast: true, position: 'top-end', icon: 'error', title: '<?= $_SESSION['error_msg'] ?>', showConfirmButton: false, timer: 3000, customClass: { popup: 'rounded-2xl' }});
